...
pygame.display.set_caption("Collision Modes")
...
enemy_img = pygame.transform.scale(enemy_img, (120, 80))
...
def sat_collision(c1, c2):
    ...
    for axis in axes:
        if not overlap(project(c1, axis), project(c2, axis)):
            return False
    return True
...
enemy = pygame.Rect(400, 300, 120, 80)
...
while True:
    ...
    # AABB
    aabb_hit = player.colliderect(enemy)

    # Circle
    p_center = player.center
    e_center = enemy.center

    p_radius = player.width / 2
    e_radius = enemy.width / 2

    circle_hit = circle_collision(p_center, p_radius, e_center, e_radius)

    # OBB
    player_corners = get_corners(player, 0)
    enemy_corners = get_corners(enemy, enemy_angle)

    obb_hit = sat_collision(player_corners, enemy_corners)
    ...
    rotated_enemy = pygame.transform.rotate(enemy_img, -math.degrees(enemy_angle))
    rotated_rect = rotated_enemy.get_rect(center=enemy.center)
    ...
    # Circle
    pygame.draw.circle(screen, BLUE, p_center, int(p_radius), 2)
    pygame.draw.circle(screen, BLUE, e_center, int(e_radius), 2)

    # OBB
    pygame.draw.polygon(screen, GREEN, player_corners, 2)
    pygame.draw.polygon(screen, GREEN, enemy_corners, 2)
